...
- **C1 addendum — the stall rule became a per-program policy (2026-08-09).** `programs.autoreg_stall_policy`: `'all-sets'` (the default — C1's ANY-scorable-set-under-floor rule, unchanged) | `'first-set'` (the top-set-driven convention, a documented user preference: only the lowest-setNumber working set governs; its miss stalls the session, 8,8,6 with the first set at its floor progresses, and an unscorable governing set yields NO verdict — silence over corruption, never a fallback set). The policy drives fixed-mode stall scoring and the M4 early-deload flag; range mode is unaffected (its stall is total-rep-gain, not floor misses). Under 'first-set' the M3 quorum for a stall verdict collapses to the governing set's own scorability; non-stall verdicts keep the half-count quorum.
