...
- **Divider row**: `flex items-center justify-between gap-4 py-4
  ...
  focus-visible:ring-ring/50` with a trailing `text-muted-foreground` value +
  `size-4` chevron cluster (`DividerRow`). Keyboard focus is the volt ring —
  the same pair buttons and inputs use — never a `bg-muted` wash: muted over
  the page background is ~1.1:1, invisible as an indicator (WCAG 2.4.7), and
  `outline-none` has already removed the UA fallback. The `/50` wash stays
  hover-only.
