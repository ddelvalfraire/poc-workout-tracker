...
import { WEEKDAY_TOKENS } from '@/lib/home/schedule-anchor'
import { metricModeSchema, type MetricMode } from '@/lib/programs/program-input'
...
/** The heaviest load this movement asks for, with its reps — the concrete
...
interface DayEditorProps {
  ...
  /** The program-wide overshoot default, so an exercise row can name what
   *  deferring actually resolves to rather than saying "default". */
  programOvershootPolicy: OvershootPolicy | null
          ...
          <div className="divide-y divide-border/60 border-t border-t-border/60">
            {day.exercises.map((exercise, exerciseIndex) => (
              <ExerciseEditor
                ...
                dispatch={dispatch}
                unit={unit}
                programOvershootPolicy={programOvershootPolicy}
              />
            ))}
          </div>
        )}
...
function ExerciseEditor({
  ...
  return (
    <div className="space-y-2 py-3">
      ...
      {/* Per-exercise override of what counts as beating the target. A SHEET
          here, where the program settings use a visible ChoiceList: four
          explained options inline, repeated for every exercise, would bury the
          day's actual content. The sheet is the density exception, and it
          still states each option's consequence rather than offering four
          bare words. */}
      <OvershootField
