...
/** Sentinel for "no explicit policy" — the radio group needs a value, and
 *  deferring to the per-scheme default is a real choice, not an absence. */
const NO_OVERSHOOT_POLICY = 'default'

const OVERSHOOT_KEYS = {
  'strict-load': 'strictLoad',
  'e1rm-equivalent': 'e1rmEquivalent',
  'any-metric': 'anyMetric',
} as const

/** Deferring leads the list, as it does everywhere else in this form. */
const OVERSHOOT_POLICIES: (OvershootPolicy | null)[] = [
  null,
  'strict-load',
  'e1rm-equivalent',
  'any-metric',
...
export function ProgramSettings({ draft, dispatch }: ProgramSettingsProps) {
  ...
  // One source of copy for both scopes — the day tree's sheet reads the same
  // namespace, so the program default and an exercise override cannot drift.
  const tOvershoot = useTranslations('OvershootField')
  ...
  return (
    <div className="space-y-6 pt-3">
      ...
      {/* What counts as beating a target, program-wide; an exercise may
          override it. A ChoiceList rather than the sheet the day tree uses:
          the set is small, closed, and every option needs a sentence — which
          is exactly what this primitive exists for. Copy comes from the
          OvershootField namespace so the two scopes cannot drift apart. */}
      <ChoiceList
        className="mt-8"
        label={tOvershoot('label')}
        value={draft.overshootPolicy ?? NO_OVERSHOOT_POLICY}
        onValueChange={(policy: string) =>
          dispatch({
            type: 'SET_OVERSHOOT_POLICY',
            value: policy === NO_OVERSHOOT_POLICY ? null : (policy as OvershootPolicy),
          })
        }
      >
        {OVERSHOOT_POLICIES.map((policy) => {
          const key = policy ?? NO_OVERSHOOT_POLICY
          return (
            <Choice
              key={key}
              value={key}
              hint={
                policy === null
                  ? tOvershoot('consequence.defaultProgram')
                  : tOvershoot(`consequence.${OVERSHOOT_KEYS[policy]}`)
              }
            >
              {policy === null
                ? tOvershoot('option.defaultProgram')
                : tOvershoot(`option.${OVERSHOOT_KEYS[policy]}`)}
            </Choice>
          )
        })}
      </ChoiceList>
