 ...
 * effort-row show rule (program sets with a prescribed effort target show the
 * row regardless). Same switch idiom as RestTimerToggle: optimistic flip with
 * rollback on failure; router.refresh so the logger pages' server-fetched
...
export function RpeLoggingToggle({ enabled }: { enabled: boolean }) {
  ...
  function toggle() {
    ...
    startTransition(async () => {
      try {
        await setRpeLoggingEnabledAction(next)
        router.refresh()
      } catch {
