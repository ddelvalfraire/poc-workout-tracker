 ...
 *     addresses a week beyond the new length. Three behaviors were on the
 *     table. Cascade-delete: a metadata edit must never silently destroy
 *     prescription data the owner deliberately pinned. Leave them orphaned:
 *     that is the exact silent-drop failure this tool exists to end — the pins
 *     would simply stop taking effect, invisibly. So: refuse, with the count
 *     and the highest pinned week in the message, and the caller clears them
 *     with `remove_program_set_override` first (or keeps the longer
 *     mesocycle). Destroying a pin stays an explicit, separately logged act.
 *     GROWING the mesocycle is always fine, and an override pinned beyond the
 *     mesocycle stays legal (setProgramSetOverride never bounded `week`) — it
 *     is simply inert until the block is long enough to reach it.
 *
 ...
 * overrides (only when the mesocycle shrinks).
 */
export async function updateProgramMeta(
  ...
  actor: ProgramEventActor,
  runIn?: PatchRunner,
): Promise<{ id: string; changed: MetaField[] } | null> {
  ...
  return (runIn ?? db).transaction(async (tx) => {
    ...
    if (values.mesocycleWeeks !== undefined && mesocycleWeeks < current.mesocycleWeeks) {
      ...
      if (orphans.n > 0) {
        const plural = orphans.n === 1 ? '' : 's'
        throw new ProgramPatchError(
          `Cannot shrink mesocycleWeeks to ${mesocycleWeeks}: ${orphans.n} per-week override${plural} pin${
            orphans.n === 1 ? 's' : ''
          } a week beyond it (highest: week ${String(orphans.maxWeek)}). Remove them with remove_program_set_override first, or keep the longer mesocycle.`,
        )
      }
    }
    ...
    await recordProgramEvent(tx, {
      ...
      payload: {
        ...
        after: Object.fromEntries(changed.map((field) => [field, values[field]])),
      },
    })
    return { id: programId, changed }
  })
}
