...
// Mins start above zero: a zero duration/distance carries no logged fact and
// is unrepresentable by the edit codecs (lib/duration.ts) — pass null to
// blank, never 0.
const durationSecArg = z.number().int().min(1).max(MAX_DURATION_SEC).nullable().optional()
const distanceMArg = z.number().min(0.01).max(MAX_DISTANCE_M).nullable().optional()

/**
 * The #206 completion gate at the tool boundary, pure-args edition: checking a
 * set off must carry evidence of the performed metric in the SAME call, so the
 * tools can never blindly complete. `argsAreAuthoritative` is true for add_set
 * (the args ARE the row, so `metricMode` absent really means the reps_weight
 * column default and a missing weight is a hard refusal); update_set passes
 * false because the row may be a cardio set or a bodyweight exercise the args
 * don't reveal — there the db layer re-validates the post-patch state against
 * the actual row (`SetCompletionError`), and this gate only refuses the
 * shapes that can NEVER be valid.
 */
function assertCompletionArgs(
  args: {
    reps?: number | null
    weight?: number | null
  ...
  argsAreAuthoritative: boolean,
): void {
  const hasWeight = typeof args.weight === 'number'
  const hasReps = typeof args.reps === 'number'
  const hasDuration = typeof args.durationSec === 'number' && args.durationSec > 0
  if (args.metricMode === 'duration' || args.metricMode === 'duration_distance') {
    if (!hasDuration) {
      throw new ToolError(
        `completing a ${args.metricMode} set requires durationSec (seconds > 0) in the same call`,
      )
    }
    return
  }
  if (argsAreAuthoritative) {
    if (!hasWeight) {
      throw new ToolError(
        'completing a reps_weight set requires a weight in the same call (pass null completed, or add the weight)',
      )
    }
    return
  }
  if (!hasWeight && !hasReps && !hasDuration) {
    throw new ToolError(
      "can't blindly complete a set: pass the performed metric (weight and/or reps, or durationSec for a cardio set) together with completed: true",
    )
  }
}
...
export function registerPatchTools(server: McpServer): void {
  server.registerTool(
    ...
    async ({ workoutId, exercisePosition, setNumber, reps, weight, completed, rir, rpe, metricMode, durationSec, distanceM, unit, userId }, extra) => {
      try {
        ...
        if (completed === true) assertCompletionArgs({ reps, weight, metricMode, durationSec }, false)
        ...
        try {
          ...
          if (error instanceof SetCompletionError) throw new ToolError(error.message)
          throw error
        }
        if (!result) {
          throw new ToolError(
  ...
  server.registerTool(
    'add_set',
    {
      title: 'Add Set',
      description:
        "Appends a set to an exercise (by workoutId + 0-based position), numbered after the current last set. reps/weight default to blank; weight is in the user's unit (or the `unit` arg). Optionally log effort with `rir`/`rpe`, or cardio fields (metricMode duration|duration_distance with durationSec seconds and distanceM meters). Pass `completed: true` to check the new set off as done — it requires the set's metric in the same call (a weight for the default reps_weight mode; durationSec > 0 for the duration modes). Returns the new set number. Errors if the workout/exercise isn't found or owned.",
      inputSchema: {
        workoutId: z.string(),
        exercisePosition: z.number().int().min(0),
        reps: repsArg,
    ...
    async ({ workoutId, exercisePosition, reps, weight, completed, rir, rpe, metricMode, durationSec, distanceM, unit, userId }, extra) => {
      try {
        ...
        assertWorkoutIdShape(workoutId)
        assertEffortArgs(rir, rpe)
        // add_set's args ARE the new row, so the completion gate is authoritative:
        // an absent metricMode really is the reps_weight column default.
        if (completed === true) assertCompletionArgs({ reps, weight, metricMode, durationSec }, true)
        const basis =
