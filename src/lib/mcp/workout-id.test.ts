...
describe('assertWorkoutIdShape', () => {
  it('does not throw for a well-formed UUID', () => {
    // Arrange
    const id = '8c2f0cc9-1a2b-4c3d-8e4f-5a6b7c8d9e0f'

    // Act + Assert
    expect(() => assertWorkoutIdShape(id)).not.toThrow()
  })

  it('accepts an uppercase UUID', () => {
    // Act + Assert
    expect(() => assertWorkoutIdShape('8C2F0CC9-1A2B-4C3D-8E4F-5A6B7C8D9E0F')).not.toThrow()
  })
  ...
  it.each([
    ...
    // Act + Assert
    expect(() => assertWorkoutIdShape(id)).toThrow(ToolError)
    expect(() => assertWorkoutIdShape(id)).toThrow(/not found/)
  })
})
