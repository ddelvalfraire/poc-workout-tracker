...
/**
 * `renderMessage` is `renderLine` under the name the view-model modules
 * adopted. Two agents built this type independently — one reached for
 * literals and nesting, the other for the plain descriptor — and both call
 * sites are correct, so the richer type wins and the other name stays as an
 * alias rather than churning twenty-one imports.
 */
export function renderMessage<K extends string>(t: Translator<K>, line: Line<K>): string
...
export function renderMessage<K extends string>(
  ...
  // Null passes through: the view-models return null for "no line here", and
  // making every caller guard before rendering would just move the check.
  return line === null ? null : renderLine(t, line)
}
